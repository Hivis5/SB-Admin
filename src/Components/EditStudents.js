...
function EditStudents() {
    ...
    let handleSave = async()=>{
        try {
            let res = await axios.put('https://61585ceb5167ba00174bbab4.mockapi.io/users/'+params.id,{name,email,mobile,degree,dept})
            console.log(res);
            navigate('/all-students')
        } catch (error) {
            console.log(error)
    ...
    let getData = async()=>{
        await fetch('https://61585ceb5167ba00174bbab4.mockapi.io/users/'+params.id,{method:'GET'})
        .then(response =>response.json())
        .then(data=>{
            setName(data.name)
            setEmail(data.email)
